---
title: "军备竞赛：从 AlexNet 到 ResNet [@he2015resnet]"
---
...
2.2 章埋的伏笔在此回收：BN 解决了深网的训练稳定性，但是，**退化（degradation）**问题浮出水面。
...
**弯路：把退化当病来治。** 2014–2015 年的主流反应是"更深需要更精心的初始化与训练术"。沿 2.1 的思路加码。方向没错但是，量级不够：退化不是调参问题。
...
2. Krizhevsky, Alex and Sutskever, Ilya and Hinton, Geoffrey E. (2012). *ImageNet Classification with Deep Convolutional Neural Networks*. *NeurIPS*. [↗ 搜索原文](https://scholar.google.com/scholar?q=Krizhevsky%202012 [@krizhevsky2012]%20ImageNet%20Classification%20with%20Deep%20Convolutional%20Neural)
...
**练习 3**：我们用的是"激活后相加"（$y = \mathrm{act}(F)+x$ 的变体）。标准 ResNet 是先加后激活；后者的恒等路也要过 relu，"纯恒等"被破坏（Pre-Act ResNet 因此，改为全程 pre-activation）。差别在深网络中显著，浅网络中难辨。
