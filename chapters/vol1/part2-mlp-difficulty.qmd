...
**"局部极小值"神话（1986–2014）**。近三十年的标准解释是：训练卡住 = 掉进了局部极小值。这个直觉来自二维地形图。看起来山谷密布。弯路在于：参数空间的维度是百万级的，而高维空间里几乎不存在"所有方向都向上"的点（一个点要有 $10^6$ 个方向全部同步向上，概率上近乎不可能）。真正遍布的是鞍点（某些方向向上、某些向下）。这个纠正是 Dauphin 等人在 2014 [@dauphin2014] 年才完成的。错怪了局部极小值三十年的代价：很多研究精力花在了"逃离局部极小"的伪问题上。
...
**训练是一场受初始落点影响的抽样**。"深度学习 = 用随机性在参数空间抽签"不是玩笑：同一配置的不同种子可以收敛到损失相差数倍的解。这本性。处理它的手段（多次运行取最好、平均权重、早停）贯穿此后二十年。
...
这就是 Goodfellow 等人 2015 年让全场吃惊的那张图：收敛点附近**一片缓慢起伏的开阔地。训练终点更像"到达了一片高原的某处"，而非"落进了谷底"。这也解释了种子彩票：不同的落点走到同一片高原的不同位置。
...
缩放版缓慢爬到 0.78，朴素版彻底躺平在 0.50。为什么除以 $\sqrt{\text{fan-in}}$ 有此神效？直觉版：一层的前向方差是 $\text{fan-in} \times \mathrm{Var}(w) \times \mathrm{Var}(a)$，想让方差逐层守恒，就需要 $\mathrm{Var}(w) \sim 1/\text{fan-in}$。这正是 Glorot 与 Bengio 2010 [@glorot2010] 年给出的分析（严格版还要同时照顾反向传播，见练习 5）。
...
浅网上两种初始化差距很小。**初始化的破坏力随深度复利增长**。这个"深度复利"结构正是下一章的主角：梯度消失不是某层的错，是每一层的小错连乘出来的总账。
...
| 问题 | 回答 |
|---|---|
| 在优化什么 loss？ | 与 1.6 相同的全批/小批 MSE。本章改变的**到达目标的路况** |
...
1. Glorot, Xavier and Bengio, Yoshua (2010). *Understanding the Difficulty of Training Deep Feedforward Neural Networks*. *Proceedings of AISTATS, 9, 249--256*. [↗ 搜索原文](https://scholar.google.com/scholar?q=Glorot%202010%20Understanding%20the%20Difficulty%20of%20Training%20Deep)
2. Dauphin et al. (2014). *Identifying and Attacking the Saddle Point Problem in High-Dimensional Non-Convex Optimization*. *NeurIPS*. [↗ 搜索原文](https://scholar.google.com/scholar?q=al.%202014%20Identifying%20and%20Attacking%20the%20Saddle%20Point)
3. Goodfellow, Ian and Bengio, Yoshua and Courville, Aaron (2016). *Deep Learning*. *MIT Press*. [↗ 搜索原文](https://scholar.google.com/scholar?q=Goodfellow%202016%20Deep%20Learning)
