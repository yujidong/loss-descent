...
上一章的断崖图（长度 8 只剩 0.64）把病灶钉在了固定向量上：解码器隔着 64 维的瓶颈望眼欲穿，而它真正需要的信息。"第 3 个源词是什么"。躺在编码器的中间状态里出不来。2014 年，Bahdanau、Cho 与 Bengio [@bahdanau2015] 的论文用一个简单的动作解决了这个问题：**别只给终态，把编码器每一步的状态都摆上桌，让解码器每走一步自己回头挑着看**。这个"回头看"的机制后来有了自己的名字。注意力（attention）。并在三年后吞并整个架构。本章先看它作为补丁的诞生。
...
`dlbook.rnn.AttentionSeq2Seq`：在 `Seq2Seq` 上加约二十行。解码每步的注意力与拼接输出。任务与上一章相同的反转"翻译"，但是，**训练集这次含长度 3–8**（分层评测才公平。原论文正是按句长分层报告 BLEU 的）：
...
典型的图像是一条干净的**反对角线**：输出第 0 个词时看输入末位，输出第 1 个词看倒数第二位……模型自己发明了"从后往前抄"的算法。没有人教过它对齐。它是从"降 loss"里长出来的。这种"无师自通的中间结构"，是本书此后反复庆祝的时刻（从词向量聚类到 induction head，同一现象的不同尺度）。
...
**Bahdanau, D., Cho, K. & Bengio, Y. (2015).** *Neural machine translation by jointly learning to align and translate.* ICLR 2015（2014 [@cho2014] 年 9 月首发 arXiv）。
...
2. 查询、键、值如果全部来自同一个序列会怎样？（自注意力。2017 年的答案）；
