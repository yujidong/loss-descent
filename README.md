...
渲染出的每一页都内置了批注工具（右下角工具栏）：

1. 点「批注模式」开启；划选正文文字后点「＋ 批注」写意见；也可以（开启模式下）直接点击某个段落对整段批注；
2. 批注保存在浏览器本地，重开页面不丢，右下角「本章批注」可查看/删除；
3. 每读完一章点「导出 JSON」，把下载的文件放进 `reviews/` 目录；
4. 运行 `python scripts/collect_review.py`，得到 `REVIEW-COMMENTS.md`（按章节分组、带原文引文与小节定位），交给我即可针对性修订。

批注类型分五种：问题 / 建议 / 文风 / 事实核对 / 其他。
