...
## ⓪ v1.2 修订完成情况(2026-09-02)

**本报告的全部条目已在 v1.2(commit 5cc5212)落地**,并经全站重渲染(0 错误)、pytest 46/46、布局自检与人工抽查复核。要点:

- 代码 5 个数学 bug 修复(SimpleRNN `Whh.T`、LSTM t=0 初态、Attention 1/√H、MLM 嵌入冻结、多输出口径),新增 10 项有限差分回归测试堵盲区;
- 全部受影响实验**重跑校准**,正文读数以实测为准——其中两处结论性修正:RNN 梯度剖面「56 个数量级」系错误梯度产物(实测 27 个数量级);bahdanau 章「attention 全线 1.00」同为假象,改写为「瓶颈不够紧则机制收益不可见」;
- 约 176 处「缓一缓」空壳全部重写;38 章手动参考文献节删除(Quarto 自动 bibliography 替代);30+ 处悬空交叉引用映射到真实章节;「?。」/断片句/术语混用清零。

**遗留事项**:
1. 【待作者拍板】`MiniGPT.clone()` 不复制 LayerNorm——这是 P0-13 跨章读数漂移的根因。v1.2 以「实现注」处理(修 clone 会改动 7.2 旗舰读数 +3.68 的成立前提,牵动一章叙事);
2. 【已完成】P2-4 插图中文题注:三卷绘图 cell 已补 `#| fig-cap`(见 v1.2.1);
3. 【规划中】C 级实验(checkpoint 加载与分析):index 已如实标注,待后续版本补一章。
...
|---|---|---|
| **R1. P0 硬伤清零** | ✅ v1.2 完成 | — |
| **R2. 实验重跑校准** | ✅ v1.2 完成(全部读数以实测为准) | — |
| **R3. 缓一缓全量重写** | ✅ v1.2 完成(约 176 处) | — |
| **R4. 文风 v1.1 落地** | ✅ v1.2 完成 | — |
| **R5. 教学补强** | ✅ v1.2 完成(RLHF 目标函数小节/BPE callout/ReAct/去剧透/REINFORCE 台阶/线性探测小实验) | — |
| **R6. 工程收尾** | ✅ v1.2 完成(参考文献去重/C 级标注/链接统一/引号 bug/代码卫生);插图题注见 v1.2.1 | — |
| **R7. 测试补盲** | ✅ v1.2 完成(pytest 46/46) | — |
